...
sentence(A,B,C,D,E) :-
%writef("sentence -- about to call declarative"),
        declarative(A,B,F,D,G),
        terminator(.,F,C,G,E).

sentence(A,B,C,D,E) :-
%writef("sentence -- about to call wh_question"),
        wh_question(A,B,F,D,G),
        terminator(?,F,C,G,E).

sentence(A,B,C,D,E) :-
%writef("sentence -- about to call yn_question"),
        yn_question(A,B,F,D,G),
%writef("sentence -- about to call terminator"),
        terminator(?,F,C,G,E)
%,writef("sentence -- terminator ? succeeded")
        .

sentence(A,B,C,D,E) :-
%writef("sentence -- about to call imperative"),
...
%writef("wh_question -- about to call variable_q"),
        variable_q(A,G,H,I,C,J,E,K),
%writef("wh_question -- about to call question"),
...
%writef("yn_question -- about to call fronted_verb, A = {0}", A),
        fronted_verb(nil,F,B,G,D,H),
%writef("yn_question -- about to call s, A={0} I={1} G={2} C={3} H={4} E={5}", [A,I,G,C,H,E]),
        s(A,I,G,C,H,E)
%,writef ("s SUCCEEDED"),
.
...
%writef("terminator -- about to call terminal, A = {0}", A),
        terminal(F,B,C,D,E),
%writef("terminator -- about to call terminator/2"),
        terminator(F,A)
%,writef("terminator -- terminator/2 succeeded")
.

terminator(A,B,C,D,E)
%:- writef("*** terminator -- FAILED with A = {0}", A), !, fail
.
